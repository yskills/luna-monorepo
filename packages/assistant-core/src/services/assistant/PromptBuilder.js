  ...
  buildSystemPrompt(user, mode = 'normal') {
    ...
    return [
      ...
      ...modeDefinition.map((line) => this.renderTemplateString(line, {
        extraInstructions: extraInstructions.join(' | ') || 'none',
        extraMemories: extraMemories.join(' | ') || 'none',
      })),
      `Current profile style: ${user.profile.style || 'playful-supportive'}.`,
      `Goals (${memoryScopeLabel}): ${(learnedGoals || []).join(' | ') || 'none'}.`,
      `Learned preferences so far (${memoryScopeLabel}): ${(learnedNotes || []).join(' | ') || 'none'}.`,
      `Long-term summaries: ${summaries || 'none'}.`,
      `Address the user personally when fitting: ${name}.`,
    ].join(' ');
  }
}
